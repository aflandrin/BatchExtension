...
- (void)automaticOfferRedeemed:(id<BatchOffer>)offer
{
    ...
    for (id<BatchResource> resource in [offer resources])
    {
		...
		NSLog(@"automaticOfferRedeemed Ressource ref= %@  value= %@", reference, [NSString stringWithFormat:@"%i", quantity]);

		sendBatchEvent("REDEEM_OFFER", "RESSOURCE", [reference UTF8String], [[NSString stringWithFormat:@"%i", quantity]UTF8String]);
    }
